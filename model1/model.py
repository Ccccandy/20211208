...
class MyModel(object):
    ...
    def __init__(self, 
        ...
        with tf.variable_scope('encoder'):
            ...
            rnn_outputs = tf.concat([rnn_fw_outputs, rnn_bw_outputs],axis=2) # B * T * D

        # CRF预测层，增加了一层testcnn、
        with tf.variable_scope('projection'):
            logits_seq = tf.layers.dense(rnn_outputs, vocab_size_bio) # B * T * V
            probs_seq = tf.nn.softmax(logits_seq)

            
            if not use_crf:
                preds_seq = tf.argmax(probs_seq, axis=-1, name="preds_seq") # B * T
            else:
        ...
        with tf.variable_scope('loss'):
            if not use_crf: 
                loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits_seq, labels=self.outputs_seq) # B * T
                
                masks = tf.sequence_mask(self.inputs_seq_len, dtype=tf.float32) # B * T
