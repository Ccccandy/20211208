...
from utils import pre_extract_kvpairs_in_bioes_type
...
def evaluate(sess, model, data_processor, vocab_dict, max_batches=None, batch_size=1024):
    ...
    while True:
        ...
        preds_seq_bio_batch, preds_seq_type_batch = sess.run(model.outputs, feed_dict)

        for pred_seq_bio, gold_seq_bio, gold_seq_attr, \
            ...
            pred_kvpair = pre_extract_kvpairs_in_bioes_type(pred_seq_bio, char_seq, pred_seq_type 
                                                        )  # (attr,type,word)
...
def predict_evaluate(sess, model, data_processor, vocab_dict, max_batches=None, batch_size=1024):
    ...
    while True:
        ...
        for pred_seq_bio, pred_seq_type, input_seq, l, eid in zip(preds_seq_bio_batch,
...
def train(tf_config, args, model, data_processor_train, data_processor_valid, vocab_dict):
    ...
    with tf.Session(config=tf_config) as sess:
        ...
        while epoches < args.num_epoch:
            ...
            train_value['loss'].append(loss)
            train_value['batch'].append(batches)
            ...
            if batches % args.evaluate_steps == 0:
                ...
                train_value['results'].append([p, r, f1])
        ...
        with open('train_data.json', 'w', encoding='utf-8') as json_file:
            json.dump(train_value, json_file, ensure_ascii=False, indent=4)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--test_input_file', '-tif', type=str, default='./data/test.json', help='Input file for prediction')
    parser.add_argument('--test_output_file', '-tof', type=str, default='submission_track1_task2.json', help='Output file for prediction')
    parser.add_argument('--do_train', '-train', action='store_true', default=True, help='Whether to run training')
    ...
    parser.add_argument('--random_seed', '-rs', type=int, default=3, help='Random seed')
